...
rule all_train:
    input:
       expand('trained_models/{dataset}/{trainer}__{plans}__{arch}/fold_{fold}.round_{i}.DONE',fold=range(5), arch=config['architecture'], dataset=config['dataset'], trainer=config['trainer'],plans=config['plans'],i=8)
...
rule train_fold_init_round:
    ...
    output:
        training_done = touch('trained_models/{dataset}/{trainer}__{plans}__{arch}/fold_{fold}.round_0.DONE')
    threads: 16
    resources:
        gpus = 1,
        mem_mb = 32000,
    ...
    shell:
        '{params.nnunet_env_cmd} && '
        '{params.rsync_to_tmp} && '
        'timeout {params.timeout} nnUNetv2_train  {wildcards.dataset} {wildcards.arch} {wildcards.fold} || true'
...
rule train_fold_round_i:
    ...
    params:
        ...
        timeout = lambda wildcards, resources: '{timeout}m'.format(timeout=(resources.time - 10))
    output:
        training_done = touch('trained_models/{dataset}/{trainer}__{plans}__{arch}/fold_{fold}.round_{i}.DONE')
    threads: 16
    resources:
        gpus = 1,
        mem_mb = 32000,
    ...
    shell:
        '{params.nnunet_env_cmd} && '
        '{params.rsync_to_tmp} && '
        'timeout {params.timeout} nnUNetv2_train --c {wildcards.dataset} {wildcards.arch} {wildcards.fold} || true'
